...
Существует два типа Gateway. Ingress Gateway предназначен для управления входящим трафиком (трафик приходящий из внешнего мира в Service Mesh). Egress Gateway используется для управления исходящим трафиком (трафик от сервисов внутри Service Mesh к внешним ресурсам).
...
Istio Gateway часто используется для терминации TLS. Он может управлять расшифровкой входящего и шифрованием исходящего трафика, а также обрабатывать TLS-сертификаты. Istio Gateway может обрабатывать несколько протоколов в проходящем трафике - HTTP, HTTPS, TCP, gRPC.
...
И манифест для Destination Rule:
