...
Важное замечание! Если на хосте установлен Docker, то нужно его удалить и перезагрузить ВМ, чтобы удалились его правила из iptables, иначе не работало. Настройка NAT не требуется! Нужно помнить про включенный IP forwarding и про обратные маршруты!!!
...
Все прекрасно работает в этой достаточно несложной схеме. Но когда лежащая в основе сетевая архитектура будет усложняться, то потребуется намного больше времени и усилий на настройку. Вместо конфигурирования маршрутов на каждом сервере, лучшим решением будет делать это на роутере, если таковой есть в вашей сети, и указать всем хостам использовать его в качестве шлюза по умолчанию. При таком подходе вы можете легко управлять маршрутами ко всем сетям в таблице маршрутизации роутера.

Вместе с этим отдельные виртуальные сети с адресом `10.244.1.0/24`, созданные нами на каждой ноде, формируют одну большую подсеть `10.244.0.0/16`.

Подведем кратко итоги. Мы выполнили некоторое количество ручных операций по подготовке окружения (с bridge-сетями и таблицами маршрутизации). Затем написали скрипт, выполняющий необходимые шаги для подключения каждого контейнера к сети, и запустили его вручную. Конечно мы не хотим повторять подобные манипуляции в больших окружениях, где тысячи pod-ов создаются ежеминутно.

Как нам запускать скрипт автоматически, когда pod создается в K8s? И вот здесь приходит CNI, выступающий в роли посредника. CNI говорит K8s как именно следует вызывать скрипт сразу после создания контейнера и говорит нам как этот скрипт должен выглядеть. Поэтому нужно немного изменить скрипт, чтобы удовлетворять стандартам CNI.

Скрипт должен иметь секцию `ADD`, которая позаботится о добавлении контейнера в сеть, и секцию `DEL`, которая позаботится об удалении интерфейсов контейнера из сети, освобождении IP-адреса и т.д.

<img src="image-3.png" width="300" height="300"><br>

Container runtime на каждой ноде отвечает за создание контейнеров. Каждый раз когда создается контейнер, container runtime смотрит в конфигурацию CNI, переданную в виде аргумента командной строки и определяет имя нашего скрипта. Затем container runtime смотрит в bin-директорию CNI для поиска скрипта и затем выполняет его с командой `ADD`, именем и namespace ID контейнера, а скрипт далее выполняет всю оставшуюся работу.
