...
Далее поговорим про ETCD. Для ETCD существуют две топологии, которые вы можете настроить в K8s. Первого варианта архитектуры, при котором ETCD является частью master-нод K8s, мы придерживались на протяжении всего курса. Он называется *Stacked Topology*, легок в установке и управлении, а также требуется меньшего количества нод. Но если одна нода уйдет в оффлайн, тогда и член ETCD и экземпляры control plane компонент будут потеряны, а избыточность подвержена риску.

<img src="image-2.png" width="600" height="350"><br>

В другом варианте архитектуры ETCD отделен от control plane нод и запускается на собственном наборе серверов. Это топология с внешними ETCD-серверами. В сравнении с предыдущей топологией она менее рискованная, т.к. упавшая control plane нода не оказывает влияния на кластер ETCD и на данные, которые он хранит. Однако он более сложный в установке и требует в два раза большего количества серверов для внешних ETCD нод.

<img src="image-3.png" width="600" height="350"><br>

Помните, что API-сервер является единственным компонентом, который взаимодействует с ETCD-сервером, и, если вы посмотрите на конфигурацию сервиса API, то у нас есть набор опций, определяющих где находится ETCD-сервер. Поэтому независимо от используемой топологии, и всякий раз, когда мы настраиваем серверы ETCD, будь они на тех же серверах или на отдельных, в конечном счете мы должны убедиться, что API-сервер настроен на правильный адрес ETCD-северов.

Запомните, что ETCD является распределенной системой, поэтому API-сервер или любой другой компонент, который хочет взаимодействовать с ETCD, может достучаться до него, обратившись к любому из его экземпляров. Вы можете читать и писать данные через любой доступный экземпляр ETCD-сервера. Вот почему мы указываем список серверов ETCD в конфигурации kube-apiserver.

<img src="image-4.png" width="900" height="450"><br>