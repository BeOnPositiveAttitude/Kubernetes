...
Хотя в лекции об этом явно и не сказано, но нужно включить forwarding, иначе связность не появится: `sysctl net.ipv4.ip_forward=1`.
...
Наконец предположим, что наша LAN подключена к Интернету и мы хотим, чтобы нашим namespace также могли ходить в Интернет.
...
А что насчет связность из внешних сетей до самих namespaces? Например в namespace `blue` работает веб-приложение на 80 порту. На данный момент namespaces находятся во внутренней частной сети и никто извне не знает о них. Мы можем пинговать их только с нашего хоста.