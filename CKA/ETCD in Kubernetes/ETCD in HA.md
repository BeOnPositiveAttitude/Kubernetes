...
Предположим у нас есть кластер из 6 нод. Из-за сбоя сеть выходит из строя, что приводит к ее разделению. Мы имеем 4 ноды в одном сегменте и 2 в другом. В этом случае группа из четырех нод имеет кворум и продолжает нормально работать. Однако в случае, если сеть разделилась другим образом, приведя к тому, что ноды равномерно распределились между двумя сегментами, каждая группа теперь имеет только 3 ноды. Но т.к. изначально у нас было 6 manager-нод, то кворум для работоспособного кластера равен 4. Соответственно ни одна из этих групп не имеет 4 manager-ов, чтобы удовлетворять условию кворума, и это приведет к падению кластера.

Поэтому при наличии четного числа нод существует возможность падения кластера при сегментации (разделении) сети.

В случае, когда мы изначально имеем нечетное число manager-ов, например 7, после разделение сети мы получим 4 ноды в одном сегменте и 3 в другом. Соответственно наш кластер все еще будет работоспособен в группе с 4 manager-ми, т.к. будет удовлетворять условию кворума.

Независимо от того каким образом разделяется сеть, больше шансов для вашего кластера остаться работоспособным при наличии нечетного числа нод.

Нечетное число нод более предпочтительно перед четным числом нод. Например 5 предпочтительнее чем 6, и в этом случае нет необходимости иметь более 5 нод, т.к. 5 уже дает достаточную отказоустойчивость.

Для установки ETCD необходимо скачать актуальную версию бинарного файла, распаковать архив, создать требуемую структуру каталогов, скопировать файлы сертификатов, выпущенных для ETCD.

<img src="image-5.png" width="650" height="250"><br>

Затем нужно настроить сервис ETCD. Здесь важно отметить опцию `--initial-cluster`, в которую мы передаем информацию о peer-ах. Так каждый сервис ETCD понимает, что он является частью кластера и где находятся его участники.

<img src="image-7.png" width="750" height="350"><br>